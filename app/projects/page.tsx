      ...
      <section className="grid grid-cols-1 md:grid-cols-12 border-b border-divider">
        ...
        <div className="md:col-span-9 p-8 md:p-16 lg:p-24">
          ...
          {loading ? (
            ...
            <div className="py-24 md:py-32 text-center border border-dashed border-divider bg-white/50">
              <h3 className="text-2xl md:text-3xl font-black uppercase mb-4">No products yet.</h3>
              <p className="label mb-8 px-6">Products will appear here once added from the admin panel.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {projects.map((project) => (
                ...
                >
                  <div className="relative aspect-video overflow-hidden bg-[#F5F5F4] border-b border-divider">
                    {project.thumbnail && !imgErrors[project.id] ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-xs text-mutedlabel uppercase tracking-widest font-bold">
                        Demo coming soon
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-6 md:p-8 flex flex-col gap-4 flex-grow">
                    <div className="flex justify-between items-start">
                      ...
                      {project.github_url && (
                        <Github className="w-5 h-5 text-mutedlabel group-hover:text-jet transition-colors flex-shrink-0" />
                      )}
                    </div>
